...
dataset_name = 'cup_only_segmentation'
...
light1, light2 = set_lights(
    ambient_strength_range  = [0.5, 20.],
    light_colour_ranges = [[0., 1.0], [0.0, 1.0], [0.5, 1.0]],
...
for num_frame in range(0, num_frames):
    ...
    if(num_frame>2):
        ...
        pos_volume = ([-0.1, -0.1, 0.15],
                      [0.1, 0.1, 0.75])  # ([0.,0.,0.15],[0.,0.,0.15]) #  ([-0.03, -0.03, 0.13], [0.03, 0.03, 0.16]) #
        orn_volume = ([0., 0., 0.], [0., 0.,np.pi*2])
...
for i in range(3):
    ...
    manip_object_category = Filter.one_by_cp(elements=MeshObject.convert_to_meshes(bpy.data.objects),
                                             cp_name='manip_object', value=True).get_cp('category_id')
    s_path = writer.make_path(save_idx, '.png', 'bottleneck_segmentation')
    writer.save_category_seg_map(seg_data['instance_segmaps'][i], seg_data['instance_attribute_maps'][i],manip_object_category, i, chunk_id,
                        path=s_path)
...
for i in range(3,8):
    ...
    manip_object_category = Filter.one_by_cp(elements= MeshObject.convert_to_meshes(bpy.data.objects) ,cp_name='manip_object',value=True).get_cp('category_id')
    s_path = writer.make_path(save_idx, '.png', 'current_segmentation')
    writer.save_category_seg_map(seg_data['instance_segmaps'][i],seg_data['instance_attribute_maps'][i],manip_object_category, i, chunk_id, path = s_path)
