...
from dense_correspondence_control.control.visual_servo import get_visual_servoing
...
cam_pos_delta = np.array([0.00,-0.00, 0.0]) #np.array([0.01,-0.01, 0.005])
cam_rot_delta = se3.so3_exp(np.array([0.,0., 0.9]).astype(float))    # se3.so3_exp(np.array([0.01,-0.02,0.01]).astype(float))
...
for num_frame in range(1, num_steps):
    ...
    #
    # twist, v_error = get_visual_servoing(starting_pixels,
    #                                     error=-f,
    #                                     depth_initial=flattened_depth,
    #                                     intrinsics=intrinsics,
    #                                     lamda = 0.1,
    #                                     # depth_bottleneck = flattened_bottleneck_depth,
    #                                     # mu = 0.01
    #                                     )

    # random_indices = np.random.choice(starting_pixels.shape[0],100,replace = False)
    # random_indices = np.arange(100)
    # starting_pixels=starting_pixels[random_indices]
    # f=f[random_indices]
    # flattened_depth= flattened_depth[random_indices]
    twist, v_error = get_visual_servoing(starting_pixels,
                                        error=-f,
                                        depth_initial=flattened_depth,
                                        intrinsics=intrinsics,
    ...
    v_errors.append(np.mean(np.linalg.norm(v_error.reshape((starting_pixels.shape)), axis =1)))

    plt.plot(v_errors)
    plt.ylim((np.min(v_errors), np.max(v_errors)))
    plt.savefig(temp_save_folder + 'v_errors.png')
    plt.close()
