...
def propagate(current_rgb,bottleneck_rgb, current_seg_map, bottleneck_seg_map, networks, image_saver, writer, num_frame, use_bottleneck_mask= True) :
    ...
    arrows_image = put_correspondence_arrows_between_images(
        np.round(255 * min_max_stardardisation_to_01(out_from[0].detach().permute(1, 2, 0).cpu().numpy())).astype(
            'uint8'),
        np.round(255 * min_max_stardardisation_to_01(out_to[0].detach().permute(1, 2, 0).cpu().numpy())).astype(
            'uint8'),
    ...
    image_saver.save_image(writer.chunk_tdir.format(chunk_id=writer.chunk_id), 'DoN_representations', num_frame,
                           don_representation)
...
cam_pos_delta = np.array([-0.07,-0.07, 0.16])
...
for num_frame in range(0, num_frames):
    ...
    networks, _ = load_network(
        '/home/eugene/Projects/dense_correspondence_control/dense_correspondence_control/learning/data/models/DoN/DoNs_randomised',
