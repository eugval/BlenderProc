...
class MyWriter:
    ...
    def __init__(self, base_path, dataset_dir_name, append_to_existing=True, has_bottleneck = False):
        ...
        self.normals_tpath = os.path.join(self.chunk_tdir, 'normals', '{im_id:06d}' + '.png')
        ...
        self.correspondances_tpath = os.path.join(self.chunk_tdir, 'correspondance', 'from_{im_id_from:06d}'+'_to_{im_id_to:06d}' + '.png')
        self.semantic_map_tpath = os.path.join(self.chunk_tdir, 'correspondance_semantic_map',   'from_{im_id_from:06d}' + '_to_{im_id_to:06d}' + '.png')
    ...
    def check_and_create_trajectory_folders(self):
        ...
        with Locker():
            ...
            if(not os.path.exists(chunk_dir)):
                ...
                os.makedirs(os.path.dirname(
                    self.normals_tpath.format(chunk_id=curr_chunk_id, im_id=0)))
                os.makedirs(os.path.dirname(
    ...
    def make_path(self, number, extension, folder, chunk_id = None):

        if (chunk_id is None):
            chunk_id = self.chunk_id

        path =  os.path.join(self.chunk_tdir.format(chunk_id = chunk_id), '{}'.format(folder), '{:06d}'.format(number) + '{}'.format(extension))
        if not os.path.exists(os.path.dirname(path)):
            os.makedirs(os.path.dirname(path))
        return path

    def save_rgb(self, rgb_image, frame_id = None, chunk_id = None, is_bottleneck = False, path = None):
        if(frame_id is None):
    ...
    def save_normals(self, normals_image, frame_id=None, chunk_id=None, is_bottleneck=False, path=None):
        if (frame_id is None):
            frame_id = self.image_id
        ...
        if (path is not None):
            ...
            normals_fpath = self.bottleneck_tpath.format(chunk_id=chunk_id, name='normals', type='.png')
        else:
            normals_fpath = self.normals_tpath.format(chunk_id=chunk_id, im_id=frame_id)

        cv2.imwrite(normals_fpath, normals_int)

    def save_depth(self, depth_image,  frame_id = None, chunk_id = None, is_bottleneck = False, path = None):
        if (frame_id is None):
            frame_id = self.image_id
    ...
    def save_seg_map(self, seg_map, attribute, frame_id = None, chunk_id = None,is_bottleneck = False, path=None):
        if (frame_id is None):
            frame_id = self.image_id
    ...
    def save_category_seg_map(self, seg_map, attribute,category_name, frame_id = None, chunk_id = None,is_bottleneck = False, path=None):
        if (frame_id is None):
            frame_id = self.image_id
        ...
        for bundle in  attribute:
            if(bundle['category_id'] == category_name):
                id_to_segment = bundle['idx']

        seg_map = (seg_map ==id_to_segment).astype('uint8')
    ...
    def get_and_save_correspondances(self,i,j, depths, camera_poses, curr_chunk_id , cam_intrinsics):
        current_d = depths[i]
        next_d = depths[j]
        current_cam_posemat = cam_from_blender_to_opencv( camera_poses[i])
        next_cam_posemat = cam_from_blender_to_opencv(camera_poses[j])
        #
