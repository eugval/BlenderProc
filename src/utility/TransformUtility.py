...
Reference : https://www.euclideanspace.com/maths/geometry/index.htm
"""
...
def tilde_vec(vec):
    """Returns the tilde_vec matrix from an x,y,z vector"""
    x,y,z = vec

    tilde_vec = np.zeros((3,3))
    tilde_vec[0,1] = -z
    tilde_vec[0,2] = y
    tilde_vec[1,0] =z
    tilde_vec[1,2] =-x
...
def gle2mat(axis,angle):
    ...
    s=np.sin(angle)
    t=1.-c
    axis_tilde = tilde_vec(axis)

    return c*np.eye(3)+ s*axis_tilde+t*(axis_tilde.dot(axis_tilde)+np.eye(3))
...
def axis_angle2quat(axis,angle):
    """
    Converts a given axis(x,y,z) and angle to a normalised quaterion
    Args:
        axis : (x,y,z) vector
        angle: float
    Returns:
    ...
    unormalised_quat = np.array([qx,qy,qz,qw])

    return unormalised_quat /np.linalg.norm(unormalised_quat)



def quat2axis_angle(quaternion):
    """
    Converts given quaternion (x,y,z,w) to axis angle representation

    Args :
    quaternon : vec4 float angles
    Returns:
        (tuple) (axis, angle) axis normalised (x,y,z)
    """

    if(quaternion[3]>1.):
        # if qw >1 there will be numerical errors - solve by normalising quaternion
    ...
    if(s < 1e-4):
        #If the angle is 0 then we get a singularity (any axis will do), so pick a normalised arbitrary axis to return
        x=1
        y=0
        z=0
    else:
...
def quat2rotvec(quaternion):
    axis,angle = quat2axis_angle(quaternion)
    return axis*angle
...
#TODO: THERE IS A BUG IN THE EULER CONVERSIONS
def eulerx2mat(angle):
    cos = np.cos(angle)
    sin = np.sin(angle)
    return np.array([[1.,0.,0.],
...
def cartesian_to_spherical(cartesian_xyz):
    '''
    returns (phi, theta ,r)
    ...
    return  np.arctan2(cartesian_xyz[1],cartesian_xyz[0]),  \
            np.arctan2(np.linalg.norm(cartesian_xyz[:2]),cartesian_xyz[2]), \
            np.linalg.norm(cartesian_xyz)
