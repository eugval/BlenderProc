...
class CorrespondancesWriter(WriterInterface):
    """ Saves the synthesized dataset. The dataset is split
    ...
    def _write_frames(self):
        ...
        for frame_id in range(bpy.context.scene.frame_start, bpy.context.scene.frame_end):
            ...
            if curr_frame_id == 0:
                ...
                chunk_camera = {}
                if not (
                os.path.exists(os.path.dirname(self.rgb_tpath.format(chunk_id=curr_chunk_id, im_id=0, im_type='PNG')))):
